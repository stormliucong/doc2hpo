function uiSearch() {
			...
			.search(
					{
						...
						apiSettings : {
							onResponse : function(res) {
								...
								// translate GitHub API response to work with
								// search
								$
										.each(
												res.terms,
												function(index, item) {
													...
													response.results
															.push({
																title : item.name
																		+ '<div class="ui right floated basic icon button" onClick="window.open(\''
																		+ href
																		+ '\', \'_blank\', \'location=yes,height=600,width=600,scrollbars=yes,status=yes\')" ><i class="zoom icon"></i></div>',
																description : item.id
	...
	$('#addHpoTerm').click(function(e) {
		...
		if (hpoArray[0] === 'NORESULTS') {
			// won't change the annotation if no results
			// returned.
			...
			var is_negated = $("#" + tagId).attr('is_negated');
			$('.prompt').val('');
			$('#addHpoTerm').hide();
			updateTermsInSession(start, length, hpo_id, hpo_term, is_negated);
		}
	});
